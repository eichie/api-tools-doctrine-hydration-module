...
class DoctrineHydratorTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $hydrateService = $hydrateService ? $hydrateService : $this->getMock('Laminas\Hydrator\HydratorInterface');
        $extractService = $extractService ? $extractService : $this->getMock('Laminas\Hydrator\HydratorInterface');
    ...
    public function it_should_extract_an_object()
    {
        ...
        $extractService = $this->getMock('Laminas\Hydrator\HydratorInterface');
        $extractService
            ->expects($this->any())
            ->method('extract')
    ...
    public function it_should_hydrate_an_object()
    {
        ...
        $hydrateService = $this->getMock('Laminas\Hydrator\HydratorInterface');
        $hydrateService
            ->expects($this->any())
            ->method('hydrate')
            ->with($data, $object)
